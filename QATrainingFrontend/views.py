...
from QATrainingFrontend.utils import authed, ip2long, long2ip, is_setup, validate_url, get_config, set_config, sha512, get_ip
from QATrainingFrontend.models import db, Users, Solves, Awards, Challenges, WrongKeys, Keys, Tags, Files, Tracking, Pages, Config
...
def setup():
    ...
    <h3 class="text-center">
        Welcome to Mirantis QA Training System</a>
    </h3>

    <h4 class="text-center">
        <a href="{0}/admin">Click here</a> to login and configure
    </h4>
...
def users(page):
    ...
    if get_config('verify_emails'):
        users = Users.query.filter_by(verified=True).slice(page_start, page_end).all()
    else:
