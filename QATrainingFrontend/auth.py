...
from QATrainingFrontend.utils import sha512, is_safe_url, authed, can_send_mail, sendmail, can_register, get_config, verify_email
from QATrainingFrontend.models import db, Users
...
def reset_password(data=None):
    ...
    if data is not None and request.method == "POST":
        ...
        user.password = bcrypt_sha256.encrypt(request.form['password'].strip())
        db.session.commit()
        db.session.close()
        return redirect(url_for('auth.login'))
    ...
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            ...
            with app.app_context():
                user = Users(name, email.lower(), password)
                db.session.add(user)
                db.session.commit()
                db.session.flush()
