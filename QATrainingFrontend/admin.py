...
from QATrainingFrontend.utils import sha512, is_safe_url, authed, admins_only, is_admin, unix_time, unix_time_millis, get_config, \
    set_config, sendmail, rmdir, create_image, delete_image, run_image, container_status, container_ports, \
    container_stop, container_start, get_themes
from QATrainingFrontend.models import db, Users, Solves, Awards, Containers, Challenges, WrongKeys, Keys, Tags, Files, Tracking, Pages, Config, DatabaseError
from QATrainingFrontend.scoreboard import get_standings
...
def admin_chals():
    if request.method == 'POST':
        ...
        users_with_points = db.session.query(Solves.userid).join(Users).filter(
        ...
        for x in chals:
            ...
            if users_with_points > 0:
                percentage = (float(solve_count) / float(users_with_points))
            else:
                percentage = 0.0
...
def email_user(userid):
    ...
    if message and user:
        if sendmail(user.email, message):
            return "1"
    return "0"
...
def admin_scoreboard():
    ...
    return render_template('admin/scoreboard.html', users=standings)
...
def create_award():
    try:
        ...
        award = Awards(userid, name, value)
...
def admin_scores():
    ...
    users = db.session.query(Solves.userid, Users.name, score).join(Users).join(Challenges).filter(Users.banned == False).group_by(Solves.userid).order_by(score.desc(), quickest)
    db.session.close()
    json_data = {'users':[]}
    for i, x in enumerate(users):
        json_data['users'].append({'place':i+1, 'id':x.userid, 'name':x.name,'score':int(x.score)})
    return jsonify(json_data)
...
@admin.route('/admin/solves/<userid>/<chalid>/delete', methods=['POST'])
@admins_only
def delete_solve(userid, chalid):
    solve = Solves.query.filter_by(userid=userid, chalid=chalid).first()
...
@admin.route('/admin/wrong_keys/<userid>/<chalid>/delete', methods=['POST'])
@admins_only
def delete_wrong_key(userid, chalid):
    wrong_key = WrongKeys.query.filter_by(userid=userid, chalid=chalid).first()
...
def admin_wrong_key(page='1'):
    ...
    wrong_keys = WrongKeys.query.add_columns(WrongKeys.chalid, WrongKeys.flag, WrongKeys.userid, WrongKeys.date,\
                Challenges.name.label('chal_name'), Users.name.label('user_name')).\
                join(Challenges).join(Users).order_by('user_name ASC').slice(page_start, page_end).all()
...
def admin_correct_key(page='1'):
    ...
    solves = Solves.query.add_columns(Solves.chalid, Solves.userid, Solves.date, Solves.flag, \
                Challenges.name.label('chal_name'), Users.name.label('user_name')).\
                join(Challenges).join(Users).order_by('user_name ASC').slice(page_start, page_end).all()
